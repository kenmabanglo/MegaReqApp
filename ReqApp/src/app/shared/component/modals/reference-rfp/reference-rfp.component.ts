...
@Component({
  selector: 'app-reference-rfp',
  templateUrl: './reference-rfp.component.html',
  styleUrls: ['./reference-rfp.component.scss']
})
export class ReferenceRfpComponent implements OnInit, AfterViewInit {
  ...
  selection = new SelectionModel<PaginatedResponse>(false, []);
    ...
    public dialogRef: MatDialogRef<ReferenceRfpComponent>
      ) { 
  ...
  initDataSource(page:number=0, size: number=10, search="") {
    this.request.findAllRFPNo(page,size,this.user.branchCode,search)
    .subscribe({
      next: (res) => {
  ...
  submit() {
    ...
    this.dialogRef.close({requestNo: selected['requestNo'],  extendedAmount: selected['extendedAmount']} );
  }
